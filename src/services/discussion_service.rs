...
use crate::models::{Discussion, DiscussionForm, DiscussionFormView};
use crate::repositories::{discussion_repo, user_repo};
use crate::Db;
...
pub async fn list_discussions_by_client(
    ...
    discussion_repo::list_discussions_by_client(db, tenant_id, client_id).await
}
...
pub async fn find_discussion_by_id(
    ...
    discussion_repo::find_discussion_by_id(db, tenant_id, client_id, discussion_id).await
}
...
pub async fn create_discussion(
    ...
    if let Err(err) = discussion_repo::create_discussion(
        db,
        tenant_id,
        client_id,
        author_id,
...
pub async fn update_discussion(
    ...
    if let Err(err) = discussion_repo::update_discussion(
        db,
        tenant_id,
        client_id,
        discussion_id,
...
pub async fn delete_discussion(
    ...
    discussion_repo::delete_discussion(db, tenant_id, client_id, discussion_id).await
}
